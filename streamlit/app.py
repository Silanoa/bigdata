...
with tabs[1]:
    ...
    df_books_mois["mois"] = df_books_mois["mois"].str.lower().map(mois_to_num)
    ...
    fig = px.imshow(
        ...
        x=pivot.columns,
        y=[calendar.month_abbr[m] for m in pivot.index],
    ...
    with col1:
        ...
        monthly_total = filtered.groupby("mois")["quantite_totale"].sum().reindex(range(1, 13)).fillna(0)
        fig = px.bar(
...
with tabs[3]:
    ...
    with col2:
        ...
        if len(annual_growth.dropna()) > 0:
            ...
            fig.update_traces(marker_color=growth_color)
            fig.update_layout(height=400)
            st.plotly_chart(fig, use_container_width=True)
        else:
    ...
    if 'unit_price' in df.columns and 'quantity' in df.columns:
        ...
        yearly_metrics['avg_order_value'] = yearly_metrics['amount_paid'] / yearly_metrics['invoice_id']
        yearly_metrics['avg_customer_value'] = yearly_metrics['amount_paid'] / yearly_metrics['customer_id']
...
with tabs[5]:
    ...
    with adv_tabs[0]:
        ...
        if 'category_id' in df_dim_books.columns and 'id' in df_dim_category.columns:
            catalogue = pd.merge(
                df_dim_books,
                df_dim_category,
                left_on='category_id',
            ...
            cat_counts = catalogue.groupby('intitule_x').size().reset_index(name='count')
